...
class SkypeBot:
	...
	def GetSpectator(self):
		#spectate.py plugin used
		user = self.context.Body.split(' ')[1:]
		if len(user) > 1:
			tmp = ""
			for item in user:
				tmp += '%20' + item
			user = tmp[3:]
		print str(user)
		self.context.Chat.SendMessage('/me ' + spectate.GetUrl(str(user)))
	...
	def PrintCommands(self):
		...
		for item in self.functions.keys():
			temp += (item + ', ')
		self.context.Chat.SendMessage('/me ' + temp[:-2])
	...
	def GetUptime(self):
		...
		self.context.Chat.SendMessage('/me {0}h:{1}m:{2}s'.format(hours, minutes, seconds))
